...
% different ways to get a more holisitic picture of what is going on inside
% the HET channel. Each plot has its own figure window.

% Make sure that the mapping data csv and COMSOL 1D simulation .txt file
% are both in the same directory as this script.

% load continuous data
mapping_data = struct2cell(data_process());

% mapping parameters
column_number = 33; % CHANGE BASED ON NUMBER OF MAPPING COLUMNS
col_dist = 0.4; % in mm; CHANGE BASED ON THE DISTANCE BETWEEN MAPPING COLUMNS
speed = 50/60; % extruder speed in mm/s, F50; CHANGE BASED ON EXTRUDER SPEED
first_point = [0; 0; 0];
wall_offset = 0.2; % in mm; CHANGE BASED ON PROBE OFFSET FROM THE WALL (DON'T ACCOUNT FOR PROBE THICKNESS)
channel_width = 11; % in mm; CHANGE BASED ON THRUSTER DIMENSIONS
...
comsol_1D = readmatrix("1D_aboveandbelow_exit_plane.txt"); % EDIT WITH FILE NAME
...
function data = data_process()
    data_matrix = readmatrix('mapping_data.csv'); % RENAME BASED ON MAPPING DATA FILE
    data = [];
